...
def predict_image_sign_language(image):
...
def index():
    return jsonify({
        "status": {
            "code": 200,
            "message": "Welcome to API Sign Language Image Prediction",
        },
        "data": None
    }), 200
...
def predict():
    if request.method == "POST":
        ...
        if image and allowed_file(image.filename):
            ...
            class_name, confidence_score = predict_image_sign_language(image_path)
            return jsonify({
                "status": {
                    "code": 200,
                ...
                "data": {
                    "image_sign_language_prediction": class_name,
